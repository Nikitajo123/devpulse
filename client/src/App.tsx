    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <div className="relative max-w-5xl mx-auto px-6 py-12 min-h-screen flex flex-col">
        ...
        <div className="flex flex-col items-center mb-16">
          ...
          <p className="text-zinc-500 text-lg tracking-wide">GitHub • AI Insights</p>
        </div>
        ...
        <div className="relative max-w-2xl mx-auto w-full mb-16">
          ...
          <p className="text-center text-zinc-500 text-sm mt-3">Discover talent • Analyze activity • AI-powered</p>
        </div>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="mt-8 text-zinc-400 text-lg font-light tracking-widest">PULSE CHECK IN PROGRESS...</p>
            <p className="text-zinc-500 text-sm mt-2">AI is scanning repositories &amp; contributions</p>
          </div>
        )}
        ...
        {error && (
          <div className="max-w-md mx-auto bg-red-950/30 border border-red-900/50 rounded-3xl p-8 text-center">
            ...
            <p className="text-zinc-500 mt-2">Make sure the username is correct</p>
          </div>
        )}
        ...
        {data && (
          <div className="space-y-12">
            ...
            <div className="bg-zinc-900/70 backdrop-blur-2xl border border-zinc-700/70 rounded-3xl p-10 flex flex-col md:flex-row gap-10 items-center md:items-start">
              ...
              <div className="flex-1 text-center md:text-left">
                <div className="flex flex-col md:flex-row md:items-end gap-3 md:gap-4">
                  <h2 className="text-4xl font-semibold tracking-tight">{data.name || data.username}</h2>
                  <p className="text-2xl text-zinc-500 font-mono">@{data.username}</p>
                </div>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-x-10 gap-y-3 mt-8 text-sm">
                  <div>
                    <span className="text-3xl font-semibold text-white">{data.followers}</span>
                    <span className="block text-xs text-zinc-500 tracking-widest mt-1">FOLLOWERS</span>
                  </div>
                  <div>
                    <span className="text-3xl font-semibold text-white">{data.following}</span>
                    <span className="block text-xs text-zinc-500 tracking-widest mt-1">FOLLOWING</span>
                  </div>
                  <div>
                    <span className="text-3xl font-semibold text-white">{data.publicRepos}</span>
                    <span className="block text-xs text-zinc-500 tracking-widest mt-1">REPOSITORIES</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {data.aiSummary && (
              <div className="relative bg-gradient-to-br from-zinc-900 to-zinc-950 border border-violet-500/30 rounded-3xl p-9 overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-5">
                  <div className="px-4 py-1 bg-violet-500/10 text-violet-400 text-xs font-mono tracking-[2px] rounded-full border border-violet-500/20">AI INSIGHT</div>
                </div>
            ...
            <div className="bg-zinc-900/70 backdrop-blur-xl border border-zinc-700/70 rounded-3xl p-9">
              <h3 className="uppercase text-xs tracking-[3px] text-zinc-500 mb-6">Primary Languages</h3>
              <div className="flex flex-wrap gap-3">
                {Object.entries(data.languages).map(([lang, count]: any) => (
                  ...
                  >
                    ...
                    <div>
                      <div className="font-medium text-white group-hover:text-emerald-300 transition-colors">{lang}</div>
            ...
            {/* Commit Activity */}
            <div className="bg-zinc-900/70 backdrop-blur-xl border border-zinc-700/70 rounded-3xl p-9">
              <h3 className="uppercase text-xs tracking-[3px] text-zinc-500 mb-7">WEEKLY COMMIT PULSE</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
                {Object.entries(data.commitDays).map(([day, count]: any) => (
                  ...
                  >
                    <div className="text-xs uppercase tracking-widest text-zinc-500 mb-3 group-hover:text-violet-400 transition-colors">{day}</div>
                    <div className="text-5xl font-semibold text-white tabular-nums">{count}</div>
                    <div className="text-[10px] text-emerald-400 mt-1 tracking-widest">COMMITS</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Top Repos */}
            <div className="bg-zinc-900/70 backdrop-blur-xl border border-zinc-700/70 rounded-3xl p-9">
              <h3 className="uppercase text-xs tracking-[3px] text-zinc-500 mb-8">STARRED REPOSITORIES</h3>
